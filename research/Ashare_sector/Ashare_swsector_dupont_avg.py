...
for sector in sectors:
    pass
    secname_list = list(df_allsec.loc[df_allsec['申万三级行业'] == sector, "名称"].values)

    excel_outpath = ROOT_PATH + "/股票/申万行业分类/行业杜邦分析/" + sector + "_median.xlsx"
    ...
    for measure in ["净资产收益率", "总资产净利润率", "归属母公司净利润占比", "权益乘数", "营业净利润率", "总资产周转率", "资产负债率"]:
        res_tbl = pd.concat(
            [res_tbl, pd.DataFrame(sec_tbl[sec_tbl.index == measure].median(axis=0), columns=[measure + "(median)"]).T],
            axis=0)
        res_tbl = pd.concat(
            [res_tbl, pd.DataFrame(sec_tbl[sec_tbl.index == measure].std(axis=0), columns=[measure + "(std)"]).T],
            axis=0)
        res_tbl.loc[measure + "(median+std)"] = res_tbl.loc[measure + "(median)"] + res_tbl.loc[measure + "(std)"]
        res_tbl.loc[measure + "(median-std)"] = res_tbl.loc[measure + "(median)"] - res_tbl.loc[measure + "(std)"]
